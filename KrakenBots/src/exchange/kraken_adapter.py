...
class KrakenAdapter(BaseExchangeAdapter):
    ...
    def get_positions(self) -> dict:
        ...
        if self.mode == "futures":
            ...
            # Spot mode: use position tracker if available
    ...
    def create_position(self, epic: str, direction: str, size: float,
        ...
        return {"dealReference": deal_ref, "fillPrice": fill_price}
    ...
    def close_position(self, position_id: str, direction: Optional[str] = None,
        ...
        if symbol and direction and size:
            ...
            try:
                params = {"reduceOnly": True} if self.mode == "futures" else {}
                order = self.exchange.create_order(
                    symbol, "market", close_side, size, params=params
                )
                fill_price = float(order.get("average", order.get("price", 0)) or 0)
    ...
    def create_batch_orders(self, orders: list) -> list:
        ...
        for order_spec in orders:
            try:
                ...
                if order_type == "limit":
                    result = self.exchange.create_limit_order(symbol, side, size, level)
                elif order_type in ("stop-loss", "take-profit"):
                    result = self.exchange.create_order(
                        symbol, order_type, side, size,
                        price=None, params={"stopPrice": level}
                    )
                else:
                    result = self.exchange.create_order(symbol, order_type, side, size, level)
