...
from src.coach.strategy_profiles import evaluate_against_expectations, get_expectations
from src.coach.leaderboard import build_leaderboard, export_leaderboard, format_leaderboard_telegram
...
class Coach:
    ...
    def __init__(self, config):
        ...
        # Coach type: "ai", "rule", "scalper", or "master"
        self.coach_type = coach_cfg.get("type", "ai")
        self.strategy_context = coach_cfg.get("strategy_context", "")

        # Data directories
        self.bot_data_dir = coach_cfg.get("bot_data_dir", os.environ.get("BOT_DATA_DIR", "bot_data"))
        self.coach_data_dir = os.environ.get("COACH_DATA_DIR", "coach_data")
        ...
        if self.strategy_context:
            self.advisor.strategy_context = self.strategy_context
        self.market_data = CoachMarketData(config)
        self.advisor.set_market_data(self.market_data)
        self.notifier = TelegramNotifier(config)

        # Discover bots (filtered by coach type for dedicated coaches)
        all_bots = discover_bots(self.bot_data_dir)
        if self.coach_type == "master":
            self.bots = {}  # Master reads from coach_data, not bot_data
        elif self.coach_type in ("rule", "scalper", "ai"):
            # Only include bots matching this coach's type
            self.bots = {
                bid: path for bid, path in all_bots.items()
                if get_bot_type(bid) == self.coach_type
            }
        else:
            self.bots = all_bots

        # Load variants config for profile-aware analysis
        self.variants_config = self._load_variants_config()

        # Register Telegram commands
        self._register_commands()

    def _load_variants_config(self):
        """Load variants.yaml for profile-aware analysis."""
        import yaml
        for search_path in ["variants.yaml", "/app/variants.yaml", "../variants.yaml"]:
    ...
    def _register_commands(self):
        ...
        self.notifier.register_command("/coach_leaderboard", self._cmd_leaderboard)
    ...
    def _cmd_leaderboard(self, args):
        """Generate and show bot leaderboard."""
        if not self.bots:
            return "⚠️ Ingen bots fundet."
        leaderboard = build_leaderboard(self.bots, self.variants_config)
        export_leaderboard(leaderboard, "data/leaderboard.json")
        return format_leaderboard_telegram(leaderboard)
    ...
    def _cmd_help(self, args):
        coach_label = {
            ...
            "ai": "AI Coach",
            "master": "Master Coach",
        }.get(self.coach_type, "Coach")

        return (
            f"🏋️ <b>{coach_label} Kommandoer</b>\n\n"
            ...
            "/coach_leaderboard — Vis bot ranking"
        )
    ...
    def _run_analysis(self, trigger="scheduled"):
        ...
        try:
            ...
            for bot_id, db_path in self.bots.items():
                ...
                vcfg = self.variants_config.get(bot_id.upper(), {})
                profile = vcfg.get("profile", "moderate")
                evaluation = evaluate_against_expectations(bot_type, profile, {
                    "trades_per_day": trade_count / max(self.analysis_days, 1),
                    "win_rate": stats.get("drawdown", {}).get("total_pl", 0),
                    "max_drawdown_pct": abs(stats.get("drawdown", {}).get("max_drawdown", 0)),
                    "profit_factor": stats.get("profit_factor", {}).get("profit_factor", 0) or 0,
                })
                stats["profile_evaluation"] = evaluation
...
def main():
    ...
    coach_label = {
        ...
        "ai": "AI Coach",
        "master": "Master Coach",
    }.get(coach.coach_type, "Coach")

    logger.info(f"[{coach_label}] Starting with {len(coach.bots)} bots: {list(coach.bots.keys())}")
