...
class BotCoordinator:
    ...
    def update_equity(self, balance: float, initial_balance: float = None,
        ...
        For margin bots, balance should be equity (includes unrealized P/L).
        margin_info provides additional margin status for monitoring.
        """
        state = self._read_state()
        ...
        # Include margin info if available
        if margin_info:
            entry["margin_level"] = margin_info.get("margin_level", 0)
            entry["unrealized_pl"] = margin_info.get("unrealized_pl", 0)
            entry["margin_used"] = margin_info.get("margin_used", 0)
        state["equity"][self.bot_id] = entry
        self._write_state(state)
