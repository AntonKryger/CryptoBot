"""
Risk Manager for KrakenBots — supports spot, spot_margin, and futures modes.

Key differences from RuleBot's RiskManager:
- Supports spot (1:1), spot_margin (2-5x), and futures leverage
...
- Margin-aware: tighter risk limits when using leverage
"""
...
class SpotRiskManager:
    """Manages position sizing, stop-loss, take-profit for spot and margin trading."""
    ...
    def __init__(self, config):
        ...
        # Leverage: spot=1, spot_margin=from config, futures=from config
        trading_cfg = config.get("trading", {})
        if self.mode in ("spot_margin", "futures"):
            self.leverage = exchange_cfg.get("leverage", 2)
        else:
            self.leverage = 1

        # Cap leverage at max_leverage for safety
        self.max_leverage = risk_cfg.get("max_leverage", 2)
        if self.leverage > self.max_leverage:
            logger.warning(f"Leverage {self.leverage}x capped at max_leverage {self.max_leverage}x")
            self.leverage = self.max_leverage

        # Margin-aware risk limits: tighter when using leverage
        if self.is_margin:
            # Margin defaults: stricter to account for amplified losses
            self.daily_loss_limit = risk_cfg.get("daily_loss_limit", 3.0)   # 3% (was 5% for spot)
            self.total_loss_limit = risk_cfg.get("total_loss_limit", 15.0)  # 15% (was 30% for spot)
        else:
            self.daily_loss_limit = risk_cfg.get("daily_loss_limit", 5.0)
            self.total_loss_limit = risk_cfg.get("total_loss_limit", 30.0)

        # Margin call warning: alert when margin level drops below this %
        self.margin_call_warning_pct = risk_cfg.get("margin_call_warning_pct", 150.0)
    ...
    def calculate_position_size(self, allocated_amount, price):
        ...
        For spot: size = allocated / price (1:1)
        For margin: size = (allocated * leverage) / price
        Risk stays the same: SL hit loses allocated_amount, not leveraged amount.
        """
        effective_amount = allocated_amount * self.leverage
        # Graduated risk: reduce size by 50% when daily loss >= reduce threshold
        ...
        if self.is_margin:
            logger.debug(f"Margin sizing: ${allocated_amount:.0f} x {self.leverage}x = ${effective_amount:.0f} → {size:.6f} units")
    ...
    def allocate_capital(self, signals, available_balance):
        ...
        For margin: available_balance should be free_margin (not leveraged).
        Leverage is applied in calculate_position_size(), not here.
        This keeps allocation % based on actual collateral, not borrowed funds.
        """
        if not signals:
            return []
    ...
    def check_margin_health(self, balance_info: dict) -> tuple:
        """Check margin level health. Returns (warning: bool, message: str).

        Kraken margin levels:
        - margin_level > 200%: healthy
        - margin_level 150-200%: caution
        - margin_level < 150%: warning (approaching margin call)
        - margin_level < 80%: Kraken starts liquidating
        """
        ...
        if margin_level < self.margin_call_warning_pct:
            return True, (
                f"MARGIN WARNING: level {margin_level:.0f}% < {self.margin_call_warning_pct:.0f}% "
                f"(used: ${balance_info.get('margin_used', 0):.2f}, "
                f"free: ${balance_info.get('free_margin', 0):.2f})"
            )

        return False, f"Margin level: {margin_level:.0f}%"
