...
class StatsEngine:
    ...
    def get_leaderboard_data(self):
        """Compute leaderboard data: per-bot metrics with composite scoring."""
        import math
        entries = []
        ...
        for bot_name in self.db_paths:
            ...
            if closed.empty:
                entries.append({
                    "bot": bot_name,
                    "trades": 0, "open": len(open_trades),
                    "win_rate": 0, "total_pl": 0, "profit_factor": 0,
                    "sharpe": 0, "max_drawdown": 0, "trades_per_day": 0,
                    "avg_win": 0, "avg_loss": 0, "best_trade": 0, "worst_trade": 0,
                    "composite_score": 0, "rank_tier": "neutral",
                })
                continue
            ...
            pf = total_win_eur / max(total_loss_eur, 0.01)
            ...
            if len(pls) >= 5:
                ...
                std_r = closed["profit_loss"].std()
                if std_r and std_r > 0:
                    sharpe = float(mean_r / std_r * (252 ** 0.5))
            ...
            if not closed.empty and "exit_timestamp" in closed.columns:
                try:
                    dates = pd.to_datetime(closed["exit_timestamp"].dropna())
                    if len(dates) >= 2:
            ...
            # Composite score: 0.30×Sharpe + 0.25×PF + 0.20×WR + 0.15×(1-DD_norm) + 0.10×volume
            # Normalize components to 0-10 scale
            sharpe_norm = max(min(sharpe / 3.0, 1), -1) * 10  # Sharpe 3.0 = max
            pf_norm = max(min(pf / 3.0, 1), 0) * 10  # PF 3.0 = max
            wr_norm = win_rate / 10  # 100% = 10
            dd_norm = max(1 - (max_dd / max(abs(total_pl) + 1, 1)), 0) * 10
            vol_norm = min(len(closed) / 50, 1) * 10  # 50 trades = max bonus
            ...
            entries.append({
                ...
                "rank_tier": "neutral",  # set below
            })

        # Assign rank tiers based on composite score percentile
        if entries:
            scores = sorted([e["composite_score"] for e in entries], reverse=True)
            n = len(scores)
            for entry in entries:
                if n <= 1:
                    entry["rank_tier"] = "neutral"
                else:
                    rank = scores.index(entry["composite_score"])
                    pct = rank / n
                    if pct < 0.25:
                        entry["rank_tier"] = "top"
                    elif pct >= 0.75:
